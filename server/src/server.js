...
const onlinePlayers = new Map(); // Track online players: socketId -> {username, avatar}
...
io.on('connection', (socket) => {
    ...
    socket.on('disconnect', () => {
        ...
        if (player) {
            ...
            console.log(`👋 ${player.username} disconnected (${onlinePlayers.size} remaining)`);
        } else {
            console.log(`⚠️ Socket ${socket.id} disconnected but not in onlinePlayers`);
        }
    });

    // Listen for user info
    socket.on('user-connected', ({ username, avatar }) => {
        onlinePlayers.set(socket.id, { username, avatar });
        // Broadcast updated player list
        io.emit('online-players-update', Array.from(onlinePlayers.values()));
        console.log(`👤 ${username} is online (${onlinePlayers.size} total)`);
    });

    // Handle user logout (without disconnecting socket)
    socket.on('user-logout', () => {
        const player = onlinePlayers.get(socket.id);
        if (player) {
            onlinePlayers.delete(socket.id);
            io.emit('online-players-update', Array.from(onlinePlayers.values()));
            console.log(`👋 ${player.username} logged out (${onlinePlayers.size} remaining)`);
        }
    });

    // Store username when joining room
    socket.on('join-room', ({ roomId, username, gameType, avatar }) => {
        socket.username = username; // Store username in socket
        socket.avatar = avatar; // Store avatar
    ...
    socket.on('global-message', ({ message, timestamp, username }) => {
        // Use username from socket or from message data
        const sender = socket.username || username;
